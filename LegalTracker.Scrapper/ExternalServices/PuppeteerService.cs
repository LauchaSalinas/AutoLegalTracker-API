...
namespace LegalTracker.Scrapper.ExternalServices
{
    public class PuppeteerService
    {
        ...
        public async Task LogIn(string user, string pass)
        {
            ...
            await _page.GoToAsync("https://notificaciones.scba.gov.ar/LoginSoloLectura.aspx");
            await _page.WaitForSelectorAsync("#txtDomicilioElectronico");
            await _page.TypeAsync("#txtDomicilioElectronico", user);
            await _page.TypeAsync("#txtpass", pass);
            await _page.WaitForTimeoutAsync(_timeout);

            await _page.ClickAsync("#modalMostrarMensaje > div > div > div.modal-footer > button:nth-child(3)");
            await _page.WaitForTimeoutAsync(_timeout);

            await _page.ClickAsync("#cmdentrar");
            await _page.WaitForTimeoutAsync(_timeout);

            if (_page != null)
            {
                Console.WriteLine("Succesfull logging");
            }
            Console.WriteLine("Failed logging");
        }
